 ...
 * @NM_UNMANAGED_INTERNAL: %TRUE when unmanaged by internal decision (eg,
 *   because NM is sleeping or exiting)
 * @NM_UNMANAGED_USER: %TRUE when unmanaged by user decision (eg, via
 *   unmanaged-specs or D-Bus). Unrecognized device types (#NMDeviceGeneric)
 *   have this flag set by default.
 */
typedef enum {
	NM_UNMANAGED_NONE     = 0x00,
	NM_UNMANAGED_INTERNAL = 0x01,
	NM_UNMANAGED_USER     = 0x02,
