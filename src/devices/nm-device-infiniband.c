...
update_connection (NMDevice *device, NMConnection *connection)
{
	...
	if (ifindex > 0) {
		if (!nm_platform_infiniband_get_info (ifindex, NULL, NULL, &transport_mode))
                    ...
                    GError **error)
{
	...
	if (   !nm_platform_infiniband_partition_add (parent_ifindex, p_key, out_plink)
	    && nm_platform_get_error () != NM_PLATFORM_ERROR_EXISTS) {
		g_set_error (error, NM_DEVICE_ERROR, NM_DEVICE_ERROR_FAILED,
		             "failed to create InfiniBand P_Key interface: %s",
...
nm_device_infiniband_class_init (NMDeviceInfinibandClass *klass)
{
	...
	NM_DEVICE_CLASS_DECLARE_TYPES(klass, NULL, NM_LINK_TYPE_INFINIBAND)
