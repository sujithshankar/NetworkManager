...
device_recheck_slave_status (NMDevice *self, NMPlatformLink *plink)
{
	...
	} else if (priv->enslaved && !plink->master)
...
{
	/* Hardware-based devices cannot be created and have no available
	 * connections until they are in the DISCONNECTED or higher states.
	 */
	if (!nm_device_is_software (self) && nm_device_get_state (self) < NM_DEVICE_STATE_DISCONNECTED)
		return FALSE;
