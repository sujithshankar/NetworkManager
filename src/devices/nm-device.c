...
static gboolean
_queue_activation_check_must_wait_carrier (NMDevice *self, NMActRequest *req)
{
	NMDevicePrivate *priv = NM_DEVICE_GET_PRIVATE (self);
	NMConnection *connection;
	guint carrier_wait_id;
	...
	if (!nm_device_connection_is_available (self, connection, TRUE)) {
		/* We don't have carrier and are still waiting for carrier.
		 *
		 * However, the device is not available on user-request. That means
		 * that the activation comes from internal, which can only succeed
		 * if we have carrier or if the connection does not requirer carrier.
		 * The activation must not wait for carrier. */
		return FALSE;
	}

	/* See if the connection would be available, if we pretend that we don't wait
	 * for carrier */
	...
	if (is_available_without_wait) {
		/* The connection is available when we wait for carrier, but it also is available
		 * if we don't wait. The activation must not wait for carrier. */
		return FALSE;
	}
...
nm_device_queue_activation (NMDevice *self, NMActRequest *req)
{
	...
	must_wait = _queue_activation_check_must_wait_carrier (self, req);

	if (!priv->act_request && !must_wait) {
	...
	priv->queued_act_request_is_waiting_for_carrier = must_wait;
                                   ...
                                   gboolean for_user_activation_request)
{
	...
	if (   for_user_activation_request
		...
		/* Connections for an explicit user activation request might only be availale after
		 * additional checking.
		 *
                            ...
                            const char *specific_object)
{
	...
	if (   for_user_activation_request
		 ...
		 *
		 * See also _queue_activation_check_must_wait_carrier() which checks if
		 * a connection is only waiting for carrier by temporary unsetting
