...
END_ADD_DEFAULT_ROUTE:
	...
	if (priv->default_route.v4_is_assumed) {
		 ...
		 * That means that even managed connections that are never-default route, can
...
update_ip_config (NMDevice *self, gboolean initial)
{
	...
	if (priv->ext_ip6_config) {
		...
		if (priv->ac_ip6_config)
			nm_ip6_config_subtract (priv->ac_ip6_config, priv->ext_ip6_config);
		if (priv->dhcp6_ip6_config)
			nm_ip6_config_subtract (priv->dhcp6_ip6_config, priv->ext_ip6_config);
		if (priv->wwan_ip6_config)
			nm_ip6_config_subtract (priv->wwan_ip6_config, priv->ext_ip6_config);
		if (priv->vpn6_config)
