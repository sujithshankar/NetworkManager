...
test_config_simple (void)
{
	...
	g_assert_cmpstr (nm_config_data_get_config_main_file (nm_config_get_data0 (config)), ==, SRCDIR "/NetworkManager.conf");
	...
	value = nm_config_data_get_value (nm_config_get_data0 (config), "extra-section", "extra-key", NULL);
	g_assert_cmpstr (value, ==, "some value");
	g_free (value);

	value = nm_config_data_get_value (nm_config_get_data0 (config), "extra-section", "no-key", &error);
	g_assert (!value);
	g_assert_error (error, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_KEY_NOT_FOUND);
	g_clear_error (&error);

	value = nm_config_data_get_value (nm_config_get_data0 (config), "no-section", "no-key", &error);
	g_assert (!value);
...
test_config_confdir (void)
{
	...
	value = nm_config_data_get_value (nm_config_get_data0 (config), "main", "extra", NULL);
	g_assert_cmpstr (value, ==, "hello");
	g_free (value);

	value = nm_config_data_get_value (nm_config_get_data0 (config), "main", "new", NULL);
	g_assert_cmpstr (value, ==, "something"); /* not ",something" */
	g_free (value);

	value = nm_config_data_get_value (nm_config_get_data0 (config), "order", "a", NULL);
	g_assert_cmpstr (value, ==, "90");
	g_free (value);
	value = nm_config_data_get_value (nm_config_get_data0 (config), "order", "b", NULL);
	g_assert_cmpstr (value, ==, "10");
	g_free (value);
	value = nm_config_data_get_value (nm_config_get_data0 (config), "order", "c", NULL);
