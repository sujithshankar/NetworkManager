                   ...
                   GError **error)
{
	...
	if (   connection_by_uuid
		...
		nm_keyfile_connection_set_path (connection_by_uuid, full_path);
		g_object_unref (connection_new);
	...
	if (connection_by_uuid) {
		...
		old_path = nm_keyfile_connection_get_path (connection_by_uuid);
		if (source)
		...
		else if (!g_strcmp0 (old_path, nm_keyfile_connection_get_path (connection_new)))
...
_paths_from_connections (GHashTable *connections)
{
	...
	while (g_hash_table_iter_next (&iter, NULL, (gpointer *) &connection)) {
		const char *path = nm_keyfile_connection_get_path (connection);
...
read_connections (NMSystemConfigInterface *config)
{
	...
	while (g_hash_table_iter_next (&iter, NULL, (gpointer *) &connection)) {
		if (   !g_hash_table_contains (alive_connections, connection)
		    && nm_keyfile_connection_get_path (connection)) {
			if (!dead_connections)
