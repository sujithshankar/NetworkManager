                   ...
                   GError **error)
{
	...
	if (!connection_new) {
		...
		if (!ignore_error) {
			if (source)
				_LOGW ("error creating connection %s: %s", nm_connection_get_uuid (source), ERR_GET_MSG (local));
			else
				_LOGW ("error loading connection from file %s: %s", full_path, ERR_GET_MSG (local));
		}
		if (connection)
	...
	if (   connection_by_uuid
		...
		if (g_strcmp0 (nm_ifcfg_connection_get_path (connection_by_uuid), full_path) != 0) {
			/* The filename changed. Update it. */
			_LOGI ("rename \"%s\" to "NM_IFCFG_CONNECTION_LOG_FMT" without other changes", nm_ifcfg_connection_get_path (connection_by_uuid), NM_IFCFG_CONNECTION_LOG_ARG (connection_new));
			nm_ifcfg_connection_set_path (connection_by_uuid, full_path);
		}
		g_object_unref (connection_new);
	...
	if (connection_by_uuid) {
		...
		old_path = nm_ifcfg_connection_get_path (connection_by_uuid);
		if (source)
		...
		else if (!g_strcmp0 (old_path, nm_ifcfg_connection_get_path (connection_new)))
...
read_connections (SCPluginIfcfg *plugin)
{
	...
	while (g_hash_table_iter_next (&iter, NULL, (gpointer *) &connection)) {
		if (   !g_hash_table_contains (alive_connections, connection)
		    && nm_ifcfg_connection_get_path (connection)) {
			if (!dead_connections)
