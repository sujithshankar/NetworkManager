                          ...
                          GError **error)
{
	...
	connection = nm_ifcfg_connection_new (source, full_path, &local, &ignore_error);
	if (!connection) {
		if (!ignore_error)
			_LOGW ("    %s", (local && local->message) ? local->message : "(unknown)");
		if (local)
                   ...
                   GError **error)
{
	...
	if (!connection) {
		...
		if (connection) {
			const char *old_path = nm_ifcfg_connection_get_path (connection);
			_LOGI ("renaming %s -> %s", old_path, full_path);
			if (out_old_path)
				*out_old_path = g_strdup (old_path);
			nm_ifcfg_connection_set_path (connection, full_path);
		}
	}
	...
	connection_new = (NMIfcfgConnection *) nm_ifcfg_connection_new (NULL, full_path, &local, &ignore_error);
	if (!connection_new) {
	...
	if (g_strcmp0 (nm_connection_get_uuid (NM_CONNECTION (connection)), nm_connection_get_uuid (NM_CONNECTION (connection_new))) != 0) {
		 ...
		 * update_connection(). */
		_LOGW ("UUID changes are not supported. Cannot update connection %s (%s)", nm_ifcfg_connection_get_path (connection_new), nm_connection_get_uuid (NM_CONNECTION (connection_new)));
		g_object_unref (connection_new);
		return;
	}
